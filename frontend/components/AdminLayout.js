...
const AdminLayout = ({ children }) => {
  ...
  const { user, authLoading } = useAuth(); // THAY ĐỔI: Lấy authLoading từ useAuth
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== 'admin') {
      ...
      router.push('/'); // CHUYỂN HƯỚNG VỀ TRANG CHỦ nếu không phải admin
    }
  ...
  const navItems = [
    ...
    // THAY ĐỔI MỚI: Đảm bảo link khớp với cấu trúc pages/admin/theory/index.js
    { name: 'Quản lý Lý thuyết', href: '/admin/theory', icon: ( 
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5s3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18s-3.332.477-4.5 1.253" />
        </svg>
      )
    },
    // THAY ĐỔI MỚI: Đảm bảo link khớp với cấu trúc pages/admin/exercise/index.js
    { name: 'Quản lý Bài tập', href: '/admin/exercise', icon: ( 
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <header className="flex justify-between items-center py-4 px-6 bg-white border-b-4 border-indigo-600 shadow-md"> {/* THAY ĐỔI: Thêm shadow-md */}
